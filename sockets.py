...
class ICMPSocket:
    ...
    def _create_packet(self, request: ICMPRequest):
        ...
        pseudo = pack('!BBHHH',8,0,0,id,sequence,) + payload
        checksum =self._checksum(pseudo)
        return pseudo[:2] + pack('!H', checksum) + pseudo[4:]
    ...
    def _parse_reply(self, packet, source, current_time):
        ...
        type, code, checksum = unpack('!BBH',packet[self._ICMP_HEADER_OFFSET :self._ICMP_ID_OFFSET])
        data=packet[self._ICMP_HEADER_OFFSET:self._ICMP_CHECKSUM_OFFSET]+packet[self._ICMP_ID_OFFSET:]
        if(self._check_data(data, checksum)):
             if type == 0:
                 id,sequence=unpack('!HH',packet[self._ICMP_ID_OFFSET:self._ICMP_PAYLOAD_OFFSET])
             else:
                  id,sequence=unpack('!HH',packet[52:56])
        else:
            return print("wrong checksum!")
